...
export default class ProductAdmin extends Component {
	...
	handleAddProduct = async (id, event) => {
		...
		try {
			const params = { id, productname: this.state.newproduct.productname };
	...
	handleDeleteProduct = async (id, event) => {
		...
		try {
			const res = await axios.delete(`${config.api.invokeUrl}/products/${id}`);
			console.log({ res });
			const updatedProducts = [...this.state.products].filter(product => product.id !== id);
			this.setState({ products: updatedProducts });
		} catch (error) {
	...
	onAddProductNameChange = event =>
		this.setState({ newproduct: { ...this.state.newproduct, productname: event.target.value } });
	...
	render() {
		console.log('inside ProductAdmin component render');
		return (
			<Fragment>
